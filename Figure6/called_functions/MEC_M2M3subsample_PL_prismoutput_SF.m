...
% this is fo2 M@M3 subsample analysis and MEC units
...
for anim=1:length(animals)  %loop through all animals and get info about each cluster
    ...
    for shank=[MECshank]  
        ...
        %load info about units generated by TSprocessSpikes_Lruntime_SF
         ...
         units_og = units;
         clear units
        load(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) 'M32CA1_PL_whenM2M3cohalign_running1sec_2sec-consec.mat']); %units
...
writetable(mu_pvaltable, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\mu_pvaltable_M32CA1_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 


%% write r csv
savepath='L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\';
if exist(savepath)==0
     mkdir(savepath);
end
...
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M2r2CA1_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M2r2CA1_3wp_ani, e_M2r2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M2r2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M2r2CA1_8wp_ani, e_M2r2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M2r2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M2r2CA1_c_ani, i_M2r2CA1_c);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M2r2CA1_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M2r2CA1_3wp_ani, i_M2r2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M2r2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M2r2CA1_8wp_ani, i_M2r2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M2r2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %M3 %%%%%%%%%%%%%%%%%%%%%%%%%%
% M32CA1_r_exc/inh
table = rval_to_prism(e_M3r2CA1_c_ani, e_M3r2CA1_c);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1_3wp_ani, e_M3r2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1_8wp_ani, e_M3r2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M3r2CA1_c_ani, i_M3r2CA1_c);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M3r2CA1_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M3r2CA1_3wp_ani, i_M3r2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M3r2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_M3r2CA1_8wp_ani, i_M3r2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_M3r2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 


%%%%%%%%%%%%%%%%%%%%%%%%%%
% M32CA1_r_exc/inh - high R
table = rval_to_prism(e_M3r2CA1highR_c_ani, e_M3r2CA1highR_c);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1highR_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1highR_3wp_ani, e_M3r2CA1highR_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1highR_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1highR_8wp_ani, e_M3r2CA1highR_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1highR_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

% M32CA1_r_exc/inh - low R
table = rval_to_prism(e_M3r2CA1lowR_c_ani, e_M3r2CA1lowR_c);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1lowR_c_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1lowR_3wp_ani, e_M3r2CA1lowR_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1lowR_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_M3r2CA1lowR_8wp_ani, e_M3r2CA1lowR_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_M3r2CA1lowR_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
...
% writematrix(i_M2mu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M2mu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
% writematrix(i_M2mu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M2mu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
% writematrix(i_M2mu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M2mu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
% writematrix(e_M2mu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M2mu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
% writematrix(e_M2mu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M2mu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
% writematrix(e_M2mu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M2mu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

writematrix(i_M3mu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M3mu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(i_M3mu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M3mu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(i_M3mu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_M3mu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 


writematrix(e_M3mu2CA1_highR_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_highR_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_highR_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_highR_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_highR_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_highR_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_lowR_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_lowR_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_lowR_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_lowR_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_M3mu2CA1_lowR_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_M3mu2CA1_lowR_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
