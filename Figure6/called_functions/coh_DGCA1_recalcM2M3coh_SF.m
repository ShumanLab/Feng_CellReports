function coh_DGCA1_1sec_recalcM2M3coh_SF(animals)
...
%             load([exp_dir '\DGCA1coh_1sec.mat'], 'align_ind_run', 'align_ind'); %use the ind to find timebin in M2M3
...
%             DGCA1_aligned_ind_run = align_ind_run;
           ...
           % M2M3coh_whenalignDGCA1 = coh_matrix_run(find(DGCA1_aligned_ind_run == 1));
          ...
          % make a new mat that hold trus for both aligned ind and run matrix
           ...
           for i = 1: length(run_matrix)
            ...
            M2M3coh_whenalignDGCA1 = coh_matrix(find(DGCA1_aligned_indandrun == 1));

        elseif group == '3wC' | group == '8wC' 
            load([exp_dir '\M2M3coh_1sec.mat'],'coh_matrix', 'coh_matrix_run', 'track','run_matrix');  
            M2M3coh_whenalignDGCA1 = coh_matrix_run;
           % manualbin = run_times;
        end

         coh_ave = nanmean(M2M3coh_whenalignDGCA1);
         if group == '3wP'
    ...
    save([savepath '\M2M3_coh_whileDGCA1Colalign_running_3secconsec.mat'],'coh_ave_3wp','coh_ave_8wp', 'coh_ave_3wc', 'coh_ave_8wc','coh_ave_8wc_ani','coh_ave_3wc_ani','coh_ave_8wp_ani','coh_ave_3wp_ani');
