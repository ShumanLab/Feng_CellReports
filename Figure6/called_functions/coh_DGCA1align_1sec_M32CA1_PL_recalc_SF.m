function coh_DGCA1align_1sec_M32CA1_PL_recalc_SF(animals)
    for a = 1:length(animals)
        ...
        for shk = 1:length(shanks)
            ...
            if group == '3wP' | group == '8wP' 
                ...
                DGCA1align_ind = align_ind_3sec_consec; %align_ind is whole track1 length, with 1 as runing + aligned
            ...
            elseif group == '3wC' | group == '8wC' 
                load([exp_dir '\DGCA1coh_1sec.mat'],'coh_matrix', 'coh_matrix_run', 'track', 'run_matrix');  
                DGCA1align_ind = run_matrix(:,1);
            end
            ...
            win_sz = 25000;
            counter = 1;
            for i = 1: length(DGCA1align_ind)
                if counter < length(DGCA1align_ind)*25000 - win_sz
                    if DGCA1align_ind(i) == 1
                        DGCA1align_runtime_ind_25000(counter:counter+win_sz-1) = 1;
                    else
                        DGCA1align_runtime_ind_25000(counter:counter+win_sz-1) = 0;
                    end
                else
                    if DGCA1align_ind(i) == 1
                        DGCA1align_runtime_ind_25000(counter:length(DGCA1align_ind)*25000) = 1;
                    else
                        DGCA1align_runtime_ind_25000(counter:length(DGCA1align_ind)*25000) = 0;
                    end
                end
            ...
            for c=1:nclusters %loop through all clusters this shank has
                if unit(c).correctch <= ch.MEC31 & unit(c).correctch >= ch.MEC32 % only process M3 cells here
                    if ~isempty(unit(c).spiketimesnew) %deal with units with no running spiketimes
                         for s = 1:length(unit(c).spiketimesnew)
                             if unit(c).spiketimesnew(s) <= length(DGCA1align_ind)*25000  %manually control I'm in track1
                                 if DGCA1align_runtime_ind_25000(unit(c).spiketimesnew(s)) ==1
                                     unit(c).CA1DGcoh_align_binary(s) = 1; %set to true
                                 else
                                     unit(c).CA1DGcoh_align_binary(s) = 0;
                                 end
                             end
                                         
                         end
                         unit(c).spiketimesnew_tracl1align = unit(c).spiketimesnew(unit(c).CA1DGcoh_align_binary(:) == 1);
                         spk=double(unit(c).spiketimesnew_tracl1align); 
            ...
            % gather the to be saved list
                ...
                if unit(c).correctch <= ch.MEC31 & unit(c).correctch >= ch.MEC32 % only process DG cells here
                    numspikes(c) = length(unit(c).spiketimesnew_tracl1align);
            ...
            save(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) 'M32CA1_PL_whenDGCA1cohalign_running1sec_2sec-consec.mat'],'units') %save final units info as shank_units.mat in animal folder under kilosort
