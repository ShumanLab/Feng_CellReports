...
function speed_subsample_DGCA1_SF(animals)
subsample_speed_ls = [];
    ...
    for a = 1:length(animals)
        ...
        if group == '3wP' | group == '8wP' 
            ...
            if exist('align_ind_3sec_consec') 
                ...
                %%%%%%%%%%%%%%%%%%%%% whole rec
               subsample_speed = speed_mat(find(DGCA1align_ind == 1));
               subsample_speed_ave = mean(subsample_speed);
               nonsubsample_speed = speed_mat(find(DGCA1align_ind == 0));
               ...
               nonsubsample_run = [];
               for i = 1: length(run_matrix)
                   if run_matrix(i,1) == 1 && DGCA1align_ind_run(i) == 0
                       nonsubsample_run(i) = 1;
                   end
                end

               %%%
               subsample_speed_run = speed_mat(find(DGCA1align_ind_run == 1));
