function coh_M2M3align_1sec_M32CA1_PL_recalc_SF(animals)
    for a = 1:length(animals)
        ...
        for shk = 1:length(shanks)
            ...
            win_sz = 25000;
            counter = 1;
            for i = 1: length(M2M3align_ind)
                if counter < length(M2M3align_ind)*25000 - win_sz
                    if M2M3align_ind(i) == 1
                        M2M3align_runtime_ind_25000(counter:counter+win_sz-1) = 1;
                    else
                        M2M3align_runtime_ind_25000(counter:counter+win_sz-1) = 0;
                    end
                else
                    if M2M3align_ind(i) == 1
                        M2M3align_runtime_ind_25000(counter:length(M2M3align_ind)*25000) = 1;
                    else
                        M2M3align_runtime_ind_25000(counter:length(M2M3align_ind)*25000) = 0;
                    end
                end
                ...
                if unit(c).correctch <= ch.MEC31 & unit(c).correctch >= ch.MEC32 % only process M3 cells here
                    if ~isempty(unit(c).spiketimesnew) %deal with units with no running spiketimes
                         for s = 1:length(unit(c).spiketimesnew)
                             if unit(c).spiketimesnew(s) <= length(M2M3align_ind)*25000  %manually control I'm in track1
                                 if M2M3align_runtime_ind_25000(unit(c).spiketimesnew(s)) ==1
                                     unit(c).M2M3coh_align_binary(s) = 1; %set to true
                                 else
                                     unit(c).M2M3coh_align_binary(s) = 0;
                                 end
                             end
                                         
                         end
                         unit(c).spiketimesnew_track1align = unit(c).spiketimesnew(unit(c).M2M3coh_align_binary(:) == 1);
            ...
            save(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) 'M32CA1_PL_whenM2M3cohalign_running1sec_2sec-consec.mat'],'units') %save final units info as shank_units.mat in animal folder under kilosort
