...
% Bin the recording to 1s bins, calc DGCA1 coh for each bin
...
for a = 1:length(animals)
    ...
    shank = [CA1DGshank, CA1DGshank]; %first one for ca1, second one for DG
    ...
    for i = 1:length(shank)
        if i == 1
            ...
            refch(i)=round(mean([ch.Hil1 ch.Hil2]));
...
%     fpass2=12;
    ...
    shankCA1DGrefLFP1 = shankCA1DGLFP(refch(1),:); % pyr
    shankCA1DGrefLFP1 = shankCA1DGrefLFP1(starttime:track1_end_sec*Fs);
    shankCA1DGrefLFP2 = shankCA1DGLFP(refch(2),:); % DG
    shankCA1DGrefLFP2 = shankCA1DGrefLFP2(starttime:track1_end_sec*Fs);

    LFP=[shankCA1DGrefLFP1; shankCA1DGrefLFP2];    
    totalchs=length(shank); %the 4 Midpyr ch from 4 HPC shanks

   
    %coh_matrix = nan(1,size(run_times,1));
    ...
    data1 = shankCA1DGrefLFP1';
    data2 = shankCA1DGrefLFP2';
    ...
    save([exp_dir '\DGCA1coh_1sec.mat'],'coh_matrix');
end