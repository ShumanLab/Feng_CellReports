function coh_DGCA1align_1sec_DG2CA1_PL_recalc_SF(animals)
        ...
        for shk = 1:length(shanks)
           ...
           for i = 1: length(run_matrix)
            ...
            for c=1:nclusters %loop through all clusters this shank has
                if unit(c).correctch <= ch.GC1 & unit(c).correctch >= ch.Hil2 % only process DG cells here
                    if ~isempty(unit(c).spiketimesnew) %deal with units with no running spiketimes
                         ...
                         unit(c).spiketimesnew_track1align = unit(c).spiketimesnew(unit(c).CA1DGcoh_align_binary(:) == 1);
            ...
            save(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) 'DGCA1_PL_whenDGCA1cohalign_running1sec_2sec-consec.mat'],'units') %save final units info as shank_units.mat in animal folder under kilosort
