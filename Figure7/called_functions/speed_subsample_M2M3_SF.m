...
function speed_subsample_M2M3_SF(animals)
subsample_speed_ls = [];
        ...
        if group == '3wP' | group == '8wP' 
            ...
            if exist('align_ind_3sec_consec') 
                ...
                speed_mat = run_matrix(:,2);
                  %%%%%%%%%%%%%%%%%%%%% whole rec
               subsample_speed = speed_mat(find(M2M3align_ind == 1));
               subsample_speed_ave = mean(subsample_speed);
               nonsubsample_speed = speed_mat(find(M2M3align_ind == 0));
               ...
               nonsubsample_run = [];
               for i = 1: length(run_matrix)
                   if run_matrix(i,1) == 1 && M2M3align_ind_run(i) == 0
                       nonsubsample_run(i) = 1;
                   end
                end

               %%%
               subsample_speed_run = speed_mat(find(M2M3align_ind_run == 1));
               ...
               nonsubsample_speed_run_ls = [nonsubsample_speed_run_ls, nonsubsample_speed_run_ave];
                ...
                clear M2M3align_ind align_ind_3sec_consec subsample_speed subsample_speed_ave nonsubsample_speed nonsubsample_speed_ave subsample_speed_run nonsubsample_speed_run
