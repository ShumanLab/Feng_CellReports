...
% INPUTS:
% M2MOcoh_1sec.mat
% Find the subsample of the bins that matches with control coh value (M2MO coh)
...
for a = 1:length(animals)
    ...
    load([exp_dir 'exp.mat']); %load each animal's exp file for animal info
    if group == '3wC' | group == '8wC'
        load([exp_dir '\M2MOcoh_1sec.mat'],'coh_matrix'); %this is all track1, not just running
        load([stim_dir animal '_runtime_1s_bin.mat'],'run_matrix'); 
        if length(run_matrix) > length(coh_matrix)
            run_matrix = run_matrix(1:length(coh_matrix),:);
        elseif length(run_matrix) < length(coh_matrix)
        ...
        counter = counter + 1;
        save([exp_dir '\M2MOcoh_1sec.mat'],'coh_matrix', 'coh_matrix_run', 'track','run_matrix');
...
for a = 1:length(animals)
    ...
    if group == '3wP' | group == '8wP' 
        ...
        save([exp_dir '\M2MOcoh_1sec.mat'],'coh_matrix', 'coh_matrix_run', 'aligned_coh_matrix', 'track','run_matrix', 'align_ind_run', 'align_ind');
        disp([animal group]);
...
for a = 1:length(animals)
    ...
    elseif group == '3wC' | group == '8wC' 
        load([exp_dir '\M2MOcoh_1sec.mat'],'coh_matrix', 'coh_matrix_run');  %deal with control doesn't have manualbin_update
...
save([savepath '\M2MO_coh_whileM2MOColalign_running.mat'],'coh_3wp_align','coh_8wp_align', 'coh_c', 'coh_ave_c_ani', 'coh_ave_8wp_ani','coh_ave_3wp_ani');
