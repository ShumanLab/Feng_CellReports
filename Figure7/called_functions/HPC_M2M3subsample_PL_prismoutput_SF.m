...
% this is for subsample analysis when align M2M3
...
for anim=1:length(animals)  %loop through all animals and get info about each cluster
    ...
    for shank=[CA1DGshank]  
        ...
        load(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) 'DGCA1_PL_whenM2M3cohalign_running1sec_2sec_consec.mat'],'units');
...
for u=1:length(FRmean)
    if numspikes(u) < 50 || refravio(u) > 0.6 %I need to change to 50 to lose less cells
...
writetable(mu_pvaltable, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\mu_pvaltable_HPC_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
...
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_DGr2CA1_3wc_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_DGr2CA1_3wp_ani, e_DGr2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_DGr2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_DGr2CA1_8wc_ani, e_DGr2CA1_8wc);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_DGr2CA1_8wc_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(e_DGr2CA1_8wp_ani, e_DGr2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\e_DGr2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_DGr2CA1_3wc_ani, i_DGr2CA1_3wc);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_DGr2CA1_3wc_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_DGr2CA1_3wp_ani, i_DGr2CA1_3wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_DGr2CA1_3wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_DGr2CA1_8wc_ani, i_DGr2CA1_8wc);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_DGr2CA1_8wc_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 

table = rval_to_prism(i_DGr2CA1_8wp_ani, i_DGr2CA1_8wp);
writetable(table, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\r\M2M3subsample\i_DGr2CA1_8wp_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
...
writematrix(i_DGmu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_DGmu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(i_DGmu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_DGmu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(i_DGmu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\i_DGmu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_DGmu2CA1_c_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_DGmu2CA1_c_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_DGmu2CA1_3wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_DGmu2CA1_3wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
writematrix(e_DGmu2CA1_8wp_clean_double, 'L:\Susie\SummerEphysHPCEC\AnalysisOutput\HPCEC_analysis\subsample_ana\new_1sec\0.5SD\mu\M2M3subsample\e_DGmu2CA1_8wp_clean_double_whenalignM2M3.csv','Delimiter',',') %first col: name; second col: pval for kuipertest/Mann-Whitney test; third col: pval for ktest (for centralization). forth col: Wwtest (for equal means) 
