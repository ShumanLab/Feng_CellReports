...
function subsample_runinit_M2M3_SF(animals)
    ...
    for a = 1:length(animals)
        ...
        if group == '3wP' | group == '8wP' 
          ...
          % below are only RUN peroid based
                ...
                subY_runinitY_loc = (M2M3align_ind_run == 1) & (runinit_matrix == 1); %find sub + runinit
                subY_runinitY = length(find(subY_runinitY_loc == 1))/length(find(runinit_matrix == 1));

                subY_runinitN_runY_loc = (M2M3align_ind_run == 1) & (run_matrix(:,1)' == 1) & (runinit_matrix == 0); %find sub + run + non runinit
                ...
                clear M2M3align_ind align_ind_3sec_consec M2M3align_ind_run run_matrix runinit_matrix subY_runinitN_runY_loc subY_runinitY_loc subY_runinitY runinitN_runY_loc subY_runinitN_runY
            end
