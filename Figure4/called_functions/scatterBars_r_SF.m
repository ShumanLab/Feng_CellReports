...
%NOTE: run ranksum test and return p value when 2 datasets are passed in;
%run 1anova and retuen p values when 3 datasets are passed in;
%related script: scatterBars_mu_SF, scatterdouble_mu_SF
%Susie 1/25/2022; 7/17/2022
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function [results, fig_bar] = scatterBars_r_SF(data, lab, colors, title_name,save_path)
...
for i = 1:len
    ...
    % comment below if don't want data points
...
title(title_name);
if len == 3
    ylabel(['r Val, p(Cv3p)' num2str(results(1,6)) ' p(Cv8p)' num2str(results(2,6)) ' p(3pv8p)' num2str(results(3,6))]);
elseif len == 2
    ylabel(['r Value, p value = ' num2str(p)]);
end

saveas(gca, fullfile(save_path, title_name), 'svg');
