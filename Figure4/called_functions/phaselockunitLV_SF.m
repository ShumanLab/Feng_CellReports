function [PL]=phaselockunitLV_SF(spikes,fLFP)
%currently takes in spike times at 25k indexing and fLFP at 1k indexing
...
for i=1:length(spikes) %for each spike find bin
    %spike = spikes(i);      
    spike=round(spikes(i)/25);%switch to 1k index from 25k index
