%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This function outputs organized units.mat which has firing property of each unit in EC
...
%7. channel_map (map the correct ch to kilo output shifted ch)
...
%Susie updated to track specific and run/nonrun seperated 6/12/2022
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function TSprocessSpikes_EC_runtime_noseiz_SF(animal, shank, track)
...
run_times_1plus = run_times(run_length >=1, :); %only run_times longer than 3s - at this point everything is in seconds
%run_times_3plus = round(run_times_3plus*25000); %convert from seconds to samples

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
% seperate track1 or 2 spike info and put into unit_run
track2 = find(position == 5); %track1 in position is 0-4.2, track2 is 4.3-8.5
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 elseif strcmp(track,'2')==1 %previously already handled when track2 doesn't exsit
     ...
     unit_track2 = {};
      for c=1:nclusters
         spiketime_track2_end = round(find(unit(c).spiketimesnew(:) <= runtimeend)); %index for at which row the spiketimes reaches to the when rantime track1 end
         spiketime_track2_end = spiketime_track2_end(end);
         spiketime_track2_start = round(find(unit(c).spiketimesnew(:) >= runtimestart)); %index for at which row the spiketimes reaches to the when rantime track1 end
...
%%
%load(['Y:\Susie\2020\Summer_Ephys_ALL\kilosort\' animal '\shank' num2str(shank) '\cells_track' track '.mat'],'unit_run') %save new unit file after these updates
...
for c=1:nclusters %loop through all clusters this shank has
    if ~isempty(unit_run(c).spiketimes)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %below is MECshank (using MECII specifically) and choose different
        %reference for each shank
        ...
        if ~isnan(refch)
            ...
            [PL]=phaselockunitLV_SF(spk,filt_data);
            %figure; polarhistogram(PL.spike_rad,18)
            MECIIthetaPL{c}=PL;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for sp=1:length(spikelim) %for each spiketime in the cluster
            ...
            for d=2:length(ds)
                if ds(d) > 0 && ds(d) <0.02  %find spiketimes within 20ms  %should I change this so it only finds the first instance?
